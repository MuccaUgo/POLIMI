...
test("activation deletes only this app's stale caches and waits before claiming clients", async () => {
  ...
  const siblingCache = "strategy-marketing-hub:https://muccaugo.github.io/OtherApp/:v1";
  for (const name of [`${prefix}v1`, currentCache, siblingCache, "other-app", "strategy-marketing-hub-v1"]) {
  ...
  claimed.resolve();
  await event.done;
  assert.deepEqual([...sw.stores.keys()], [currentCache, siblingCache, "other-app", "strategy-marketing-hub-v1"]);
});

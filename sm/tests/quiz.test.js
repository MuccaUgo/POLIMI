...
function loadApp(storage = new Map(), random = 0.25, locks = undefined) {
  ...
  const saved = mode => JSON.parse(storage.get("sm_session_" + mode));
...
test("exam cannot finish with gaps; a completed session is counted only once", () => {
  ...
  assert.equal(app.storage.has("sm_stats"), false);
  for (let i = 0; i < 9; i++) { app.click("opt-A"); app.click("nextBtn"); }
  app.click("nextBtn");
  const stats = JSON.parse(app.storage.get("sm_stats"));
  assert.equal(stats.totalAnswered, 10);
  assert.equal(stats.totalCorrect, 9);
  assert.equal(JSON.parse(app.storage.get("sm_mistakes")).length, 1);
  assert.equal(app.storage.has("sm_session_exam"), false);
  app.click("nextBtn");
  assert.equal(JSON.parse(app.storage.get("sm_stats")).totalAnswered, 10);
});
...
test("a partial full test counts only answers and stores readable mistake text", () => {
  ...
  const mistake = JSON.parse(app.storage.get("sm_mistakes"))[0];
...
test("invalid saved sessions and malformed optional history do not crash startup", () => {
  const storage = new Map([
    ["sm_session_full", '{"version":1,"questionIds":[999]}'],
    ["sm_stats", "null"], ["sm_mistakes", "{}"]
  ]);
...
test("the same saved session completed in two tabs records results once", () => {
  ...
  second.jump(12);
  assert.equal(first.storage.has("sm_session_full"), false);
  second.click("nextBtn");
  const stats = JSON.parse(first.storage.get("sm_stats"));
  assert.equal(stats.totalAnswered, 1);
  assert.equal(stats.completedSessions.length, 1);
  assert.equal(JSON.parse(first.storage.get("sm_mistakes")).length, 1);
});
...
test("shared browser locks serialize duplicate completions across tabs", async () => {
  ...
  await queue;
  assert.deepEqual(names, ["sm_record_completion", "sm_record_completion"]);
  assert.equal(JSON.parse(first.storage.get("sm_stats")).totalAnswered, 1);
  assert.equal(JSON.parse(first.storage.get("sm_mistakes")).length, 1);
  assert.equal(first.storage.has("sm_session_full"), false);
});
